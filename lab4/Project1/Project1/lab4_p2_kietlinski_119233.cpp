...
public:
	double pop() {
		cout << "Liczba elementow na stosie: " << T::top << endl;
		if (T::top == 0)
			throw BrakDanych(typeid(T::stos[0]).name());
		double buf = T::stos[--T::top];
		cout << "Zdejmowana ze stosu wartosc to: " << buf << endl;
		return buf;
	}
};
...
public:
	...
	void push(const double& i) {
		...
		T::stos[T::top++] = i;
		suma += i;
		cout << "Aktualna suma to: " << suma << endl;
	}
...
int main() {
	...
	cout << endl<< "Domieszka 2: " << endl << endl;
	domieszka2<Stos2> d2;
	for (int i = 1; i < 5; i++)
		d2.push(i * 10);
	for (int i = 1; i < 5; i++)
